...
const Player = () => {
  ...
  const [audioFormat, setAudioFormat] = useState(
    localStorage.getItem("audioQuality") ?? "high"
  );
 ...
 const handleNext = () => {
  const state = store.getState().currentSongSlice;

  // ✅ If history has next
  if (state.currentIndex < state.history.length - 1) {
    dispatch(playNextSong());
  } 
  // ✅ Else play from Up Next list
  else if (songsList.length > 0) {
    const nextSong = songsList[0];

    if (nextSong?.videoId) {
      dispatch(addSongInfo({ id: nextSong.videoId }));
    }
  } 
  // ✅ If nothing available
  else {
    setAlertMessage("No next song available");
  }
